...
public class SuspectMask extends DelegateNode {	
	
	private final TimeSeriesMask[] masks;

	protected SuspectMask(TsDB tsdb, Node source, TimeSeriesMask[] masks) {
		super(tsdb, source);
		throwNull(masks);
		this.masks = masks;
	}
	...
	public static Node of(TsDB tsdb, Node source) {		
		...
		for (int i = 0; i < sensorNames.length; i++) {
			masks[i] = tsdb.streamStorage.getTimeSeriesSuspectMask(sourceName, sensorNames[i]);
			if(masks[i]!=null) {
				mask_counter++;
			}
		}
		...
		if(mask_counter>0) {
			return new SuspectMask(tsdb,source,masks);
		} else {
			return source;
		}
	}
