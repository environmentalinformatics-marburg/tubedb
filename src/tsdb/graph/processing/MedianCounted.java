...
import tsdb.iterator.MedianCountIterator;
import tsdb.util.iterator.TsIterator;

/**
 * This node creates median values per time step over all sources.
 * @author woellauer
 */
public class MedianCounted extends Continuous.Abstract {	

	private final List<Continuous> sources; // not null
	private final String[] inputSchema; // not null
	private final String[] outputSchema; // not null
	private final int minCount;
	private final boolean withQualityMeasures;
	private final boolean _constant_timestep;

	public MedianCounted(TsDB tsdb, List<Continuous> sources, String[] schema, int minCount, boolean withQualityMeasures) {
		super(tsdb);
		...
		this.outputSchema = MedianCountIterator.createOutputSensorNames(schema);
	}

	public static MedianCounted of(TsDB tsdb, List<Continuous> sources, int minCount, boolean withQualityMeasures) {		
		...
		return new MedianCounted(tsdb, sources, schemaSet.toArray(new String[0]), minCount, withQualityMeasures);
	}
	...
	public TsIterator getExactly(long start, long end) {		
		...
		return new MedianCountIterator(inputSchema, iteratorList.toArray(new TsIterator[0]), minCount, withQualityMeasures);		
	}
