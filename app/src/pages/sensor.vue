<template>
  <q-page class="column">
    ...
    <div class="column processing-node" v-if="sensor.aggregation_hour !== 'none' && sensor.aggregation_day !== 'none' && sensor.aggregation_week !== 'none'">
      ...
      <div class="property-grid">
        <template v-if="sensor.post_week_func !== undefined">
          <div>post_week_func</div>
          <div>{{sensor.post_week_func}}</div>
          <template v-if="sensor.post_week_func_parsed !== undefined">
            <div>--> parsed</div>
            <div>{{sensor.post_week_func_parsed}}</div>
            <!--<div> tree</div>
            <div><formula-tree :node="sensor.post_week_func_tree" :level="0"/></div>-->
            <div>--> print</div>
            <div class="formula-print">
              <formula-print :node="sensor.post_week_func_print" :level="0"/>
            </div>
          </template>
        </template>
      </div>
    ...
    <div class="column processing-node" v-if="sensor.aggregation_hour !== 'none' && sensor.aggregation_day !== 'none' && sensor.aggregation_month !== 'none'">
      ...
      <div class="property-grid">
        <template v-if="sensor.post_month_func !== undefined">
          <div>post_month_func</div>
          <div>{{sensor.post_month_func}}</div>
          <template v-if="sensor.post_month_func_parsed !== undefined">
            <div>--> parsed</div>
            <div>{{sensor.post_month_func_parsed}}</div>
            <!--<div> tree</div>
            <div><formula-tree :node="sensor.post_month_func_tree" :level="0"/></div>-->
            <div>--> print</div>
            <div class="formula-print">
              <formula-print :node="sensor.post_month_func_print" :level="0"/>
            </div>
          </template>
        </template>
      </div>
    ...
    <div class="column processing-node" v-if="sensor.aggregation_hour !== 'none' && sensor.aggregation_day !== 'none' && sensor.aggregation_month !== 'none' && sensor.aggregation_year !== 'none'">
      ...
      <div class="property-grid">
        <template v-if="sensor.post_year_func !== undefined">
          <div>post_year_func</div>
          <div>{{sensor.post_year_func}}</div>
          <template v-if="sensor.post_year_func_parsed !== undefined">
            <div>--> parsed</div>
            <div>{{sensor.post_year_func_parsed}}</div>
            <!--<div> tree</div>
            <div><formula-tree :node="sensor.post_year_func_tree" :level="0"/></div>-->
            <div>--> print</div>
            <div class="formula-print">
              <formula-print :node="sensor.post_year_func_print" :level="0"/>
            </div>
          </template>
        </template>
      </div>
